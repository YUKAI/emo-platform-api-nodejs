...
apiClient.putConversationEndpoint(serviceUuid, {endpoint: 'https://example.com/webhook2'}, {channelUser: process.env.CHANNEL_USER})
  .then(response => {
    console.log('putConversationEndpoint success')
    // console.log(response)
  })
  .catch((error: AxiosError) => {
    console.error(`ステータスコード: ${error?.response?.status}`)
